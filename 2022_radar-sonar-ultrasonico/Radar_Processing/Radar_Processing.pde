 ...
 * Autor: Saulo Aislan
 * Visualização radar para o projeto Radar/Sonar Ultrassônico com Arduino
 *
 * Artigo: https://blog.eletrogate.com/radar-sonar-ultrassonico-para-seus-projetos/
 *
 * IMPORTANTE: altere "COM5" abaixo para a porta COM do seu Arduino.
 * Feche o Serial Monitor do Arduino IDE antes de executar este sketch.
 */
...
void setup() {
  size(1366, 700);
  smooth();
  myPort = new Serial(this, "COM5", 9600);  // << Altere para sua porta COM
  myPort.bufferUntil('.');
}
...
void drawText() {
  ...
  text("Ângulo: " + iAngle + " °", 1050, 1050);
  text("Distância: ", 1380, 1050);
  if (iDistance < 40) {
